...
func TestGroupDevicesByNUMA_SLITAware(t *testing.T) {
	numa0 := int64(0)
	numa1 := int64(1)
	numa5 := int64(5)

	devices := []TopologyDevice{
		// GPU on NUMA 0, scalar only
		{DriverName: "gpu.amd.com", NUMANode: &numa0},
		// GPU on NUMA 1, scalar only
		{DriverName: "gpu.amd.com", NUMANode: &numa1},
		// NIC on NUMA 0 with SLIT list [0, 1] — equidistant to both
		{DriverName: "sriov", NUMANode: &numa0, NUMANodes: []int64{0, 1}},
		// NIC on NUMA 5, scalar only (no SLIT list)
		{DriverName: "sriov", NUMANode: &numa5},
	}

	groups := groupDevicesByNUMA(devices)

	// NUMA 0: GPU + NIC (both native)
	assert.Len(t, groups["0"], 2)
	// NUMA 1: GPU (native) + NIC (via SLIT from NUMA 0)
	assert.Len(t, groups["1"], 2)
	// NUMA 5: NIC only (scalar)
	assert.Len(t, groups["5"], 1)
}

func TestGroupDevicesByNUMA_ScalarFallback(t *testing.T) {
	numa2 := int64(2)

	devices := []TopologyDevice{
		// Device with only scalar NUMANode, no list
		{DriverName: "gpu.amd.com", NUMANode: &numa2},
		// Device with single-element list (same as scalar)
		{DriverName: "dra.cpu", NUMANode: &numa2, NUMANodes: []int64{2}},
	}

	groups := groupDevicesByNUMA(devices)

	// Both should be in NUMA 2 only
	assert.Len(t, groups["2"], 2)
	assert.Len(t, groups, 1, "should only have one NUMA group")
}

func TestPartitionBuilder_SLITAwareNUMAGrouping(t *testing.T) {
	model := NewTopologyModel()
	rules := NewTopologyRuleStore()
	builder := NewPartitionBuilder(model, rules)

	// 8 GPUs, one per NUMA, each with SLIT list showing same-socket equidistance
	gpuDevices := make([]resourcev1.Device, 8)
	pcieRoots := []string{"pci0", "pci1", "pci2", "pci3", "pci4", "pci5", "pci6", "pci7"}
	for i := 0; i < 8; i++ {
		var numaList []int64
		if i < 4 {
			numaList = []int64{int64(i), int64((i + 1) % 4), int64((i + 2) % 4), int64((i + 3) % 4)}
		} else {
			numaList = []int64{int64(i), int64(4 + (i+1)%4), int64(4 + (i+2)%4), int64(4 + (i+3)%4)}
		}
		gpuDevices[i] = resourcev1.Device{
			Name: makeDeviceName("gpu", i),
			Attributes: map[resourcev1.QualifiedName]resourcev1.DeviceAttribute{
				resourcev1.QualifiedName(AttrNUMANode): {IntValues: numaList},
				resourcev1.QualifiedName(AttrPCIeRoot): {StringValue: strPtr(pcieRoots[i])},
			},
		}
	}
	model.UpdateFromResourceSlice(makeResourceSlice("gpu-slice", "gpu.amd.com", "node-1", "gpu-pool", gpuDevices))

	// 4 NIC VFs on NUMA 0 (SLIT [0,1,2,3]) and 4 on NUMA 5 (SLIT [5,4,6,7])
	nicDevices := make([]resourcev1.Device, 8)
	for i := 0; i < 4; i++ {
		nicDevices[i] = resourcev1.Device{
			Name: makeDeviceName("nic-s0", i),
			Attributes: map[resourcev1.QualifiedName]resourcev1.DeviceAttribute{
				resourcev1.QualifiedName(AttrNUMANode): {IntValues: []int64{0, 1, 2, 3}},
				resourcev1.QualifiedName(AttrPCIeRoot): {StringValue: strPtr("pci0")},
			},
		}
		nicDevices[4+i] = resourcev1.Device{
			Name: makeDeviceName("nic-s1", i),
			Attributes: map[resourcev1.QualifiedName]resourcev1.DeviceAttribute{
				resourcev1.QualifiedName(AttrNUMANode): {IntValues: []int64{5, 4, 6, 7}},
				resourcev1.QualifiedName(AttrPCIeRoot): {StringValue: strPtr("pci5")},
			},
		}
	}
	model.UpdateFromResourceSlice(makeResourceSlice("nic-slice", "sriov", "node-1", "nic-pool", nicDevices))

	results := builder.BuildPartitions()
	require.Len(t, results, 1)

	// Count NUMA partitions that include NICs
	numaWithNICs := 0
	for _, p := range results[0].Partitions {
		if p.Type != PartitionNUMA {
			continue
		}
		if p.DeviceCounts["sriov"] > 0 {
			numaWithNICs++
		}
	}

	// All 8 NUMA partitions should have NICs (via SLIT equidistance)
	assert.Equal(t, 8, numaWithNICs, "all NUMA partitions should include NICs via SLIT")
}
